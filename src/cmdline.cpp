...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
	...
	switch (key) {
		...
		case 'a':
			if ARG_IS("bubble") {
				args->pixelsorter.algo = BubbleSort(SortingAlgorithm::BRIGHTNESS);
			} else {
		...
		case 'p': case 'd':
			if ARG_IS("up")
				args->pixelsorter.pathing = LinePath(LinePath::Direction::UP);
			else if ARG_IS("down")
				args->pixelsorter.pathing = LinePath(LinePath::Direction::DOWN);
			else if ARG_IS("left")
				args->pixelsorter.pathing = LinePath(LinePath::Direction::LEFT);
			else if ARG_IS("right")
				args->pixelsorter.pathing = LinePath(LinePath::Direction::RIGHT);
			else {
